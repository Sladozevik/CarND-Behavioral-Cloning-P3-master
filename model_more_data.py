        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                for i in range(3):                              # With this look we use all images in dataset (Center,Left,Right)
                    ...
                    if file_name_split[3] == '18':
                        data_path = './data/IMG/' + '_'.join(file_name_split)
                        image = preprocess(read_image_gray_norma(data_path))
                        # print('18',data_path)
                    else:
                        my_data_path = './my_data/IMG/' + '_'.join(file_name_split)
                        image = preprocess(read_image_gray_norma(my_data_path))
                        # print('12',my_data_path)
                    images.append(image)
